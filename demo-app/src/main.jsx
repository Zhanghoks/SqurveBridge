...
function Topbar({ health, capabilities, refresh, busy, hosted = false, showProviderConfig = true, sessionSqlAuth = false, sqlAuth, onSqlAuthChange }) {
  ...
  return <header className="topbar"><div><strong>Squrve-native runtime</strong><span>{hosted ? 'Hugging Face Space' : 'Local demo session'}</span></div><div className="runtime-state"><Status tone={tone}>{label}</Status>{sessionSqlAuth && <button className="button compact secondary" type="button" onClick={() => setSqlAuthOpen(true)}>Configure LLM</button>}{showProviderConfig && <ProviderConfig health={health} capabilities={capabilities} refresh={refresh} />}<button className="icon-button" onClick={refresh} disabled={busy} title="Refresh runtime" aria-label="Refresh runtime">↻</button></div>{sessionSqlAuth && <SqlAuthDialog open={sqlAuthOpen} api={api} status={sqlAuth} onStatusChange={onSqlAuthChange} onClose={() => setSqlAuthOpen(false)} />}</header>
}
...
function Studio({ health, capabilities, databases, selectedDb, setSelectedDb, showAgentHarness = true, hosted = false, sqlAuth }) {
  ...
  return <div className="workspace studio-workspace"><PageHeading eyebrow="Squrve run configuration" title="SQL Studio" status={<Status tone={error ? 'danger' : result ? 'success' : busy ? 'running' : 'neutral'}>{phase}</Status>} />
    <div className="config-builder-grid"><section className="tool-panel config-block"><div className="config-step"><i>01</i><div><span>Database</span><small>Execution target</small></div></div><DatabaseSelector databases={databases} value={selectedDb} onChange={setSelectedDb} />{currentDb ? <ul className="schema-chips">{currentDb.tables.slice(0, 8).map(table => <li key={table}>{table}</li>)}</ul> : <Empty title="Select an integrated database" />}</section>
      <section className="tool-panel config-block"><div className="config-step"><i>02</i><div><span>LLM Provider</span><small>{hosted ? 'Browser session credential' : 'Credential stays in local .env'}</small></div></div>{hosted ? <div className="session-auth-summary"><span>{sqlAuth?.configured ? 'Connected for this session' : 'Configure LLM from the runtime header'}</span>{sqlAuth?.configured && <><strong>{sqlAuth.provider}</strong><code>{sqlAuth.model}</code></>}<Status tone={sqlAuth?.configured ? 'success' : 'danger'}>{sqlAuth?.configured ? 'session credential available' : 'credential required'}</Status></div> : <><label className="field"><span>Provider</span><select value={provider} onChange={event => { const next = event.target.value; setProvider(next); const catalog = providers.find(item => item.id === next); setModel(catalog?.default_model || '') }}>{providers.map(item => <option key={item.id} value={item.id}>{item.id}{item.configured ? ' · configured' : ' · setup required'}</option>)}</select></label><label className="field model-id-field"><span>Model</span><input aria-label="Model" value={model} onChange={event => setModel(event.target.value)} list="studio-llm-model-suggestions" placeholder="Enter a model ID" autoComplete="off" spellCheck="false" /><datalist id="studio-llm-model-suggestions">{(providerConfig?.models || []).map(item => <option key={item} value={item} />)}</datalist><small>Choose a suggestion or enter any model ID supported by this provider.</small></label><Status tone={providerConfig?.configured ? 'success' : 'danger'}>{providerConfig?.configured ? 'credential available' : 'credential required'}</Status></>}</section>
