...
function Topbar({ health, capabilities, refresh, busy }) {
  ...
  return <header className="topbar"><div><strong>Squrve-native runtime</strong><span>Local demo session</span></div><div className="runtime-state"><Status tone={tone}>{label}</Status><ProviderConfig health={health} capabilities={capabilities} refresh={refresh} /><button className="icon-button" onClick={refresh} disabled={busy} title="Refresh runtime" aria-label="Refresh runtime">↻</button></div></header>
}
...
function WorkspaceStudio({ capabilities, databases }) {
...
function DatabaseManager({ databases, selectedDb, setSelectedDb, refreshDatabases }) {
...
function App() {
  ...
  useEffect(() => { refresh() }, [])
  return <div className="app-shell"><ShellNav page={page} setPage={setPage} /><main><Topbar health={health} capabilities={capabilities} refresh={refresh} busy={busy} />{page === 'board' ? <ExperimentBoard {...{ capabilities, api, postJson, Status, PageHeading, Empty }} /> : page === 'archive' ? <Archive {...{ api, Status, PageHeading, Empty }} /> : <WorkspaceStudio {...{ capabilities, databases }} />}</main></div>
}
