...
| Field | Value |
|---|---|
| Config | `reproduce/configs/bull-cn/e-sql.json` |
| Dataset | `bull-cn` |
| Method | `e-sql` |
| Run identifier | `bull-cn-e-sql` |
...
```bash
python reproduce/run.py bull-cn e-sql
```
...
| --- | --- | --- | --- | --- |
| esql_generate | GenerateTask | generate_type=ESQLGenerator | execute_accuracy | `../files/datasets/bull_cn_e_sql_esql_generate.json` |

## Outputs

| Name | Kind | Path |
| --- | --- | --- |
| esql_generate | stage | `../files/datasets/bull_cn_e_sql_esql_generate.json` |
...
```bash
python tools/verify.py reproduce-contract --path reproduce/configs/bull-cn/e-sql.json
```
