...
| Field | Value |
|---|---|
| Config | `reproduce/configs/spider/resdsql.json` |
| Dataset | `spider` |
| Method | `resdsql` |
| Run identifier | `spider-resdsql` |
...
```bash
python reproduce/run.py spider resdsql
```
...
| --- | --- | --- | --- | --- |
| resdsql_parse | ParseTask | parse_type=RESDSQLParser | parse_recall, parse_precision, parse_exact_matching | `../files/datasets/spider_resdsql_resdsql_parse.json` |
...
```bash
python tools/verify.py reproduce-contract --path reproduce/configs/spider/resdsql.json
```
