...
from reproduce.evolve.experience import append_failed_pattern, append_success
...
def _append_review_experience(
    ...
    """Persist the reviewed pattern so later searches can warm-start from it.

    Accepted runs land under Successful Patterns and rollbacks under Failed
    Patterns; a plain "continue" carries no reusable verdict and is skipped.
    The ``- Action: <id>`` line is the machine-readable anchor consumed by
    ``experience.failed_action_ids_from_memory``.
    """
    normalized = _normalize_review_outcome(outcome)
    if normalized == "continue":
        return
    action_id = _best_node_action_id(journal, best_node_id)
    body_lines = [
        f"- Action: {action_id or 'unknown'}",
        f"- Best node: {best_node_id or 'unknown'}",
        f"- Recommendation: {recommendation}",
    ]
    if summary:
        body_lines.append(f"- Summary: {summary}")
    body = "\n".join(body_lines)
    title = str(journal.get("evolve_slug") or "unknown-evolve")
    if normalized == "accept":
        append_success(memory_path, title=title, body=body)
    else:
        append_failed_pattern(memory_path, title=title, body=body)


def _best_node_action_id(journal: dict[str, Any], best_node_id: str | None) -> str | None:
    for node in journal.get("nodes") or []:
        if node.get("node_id") == best_node_id:
